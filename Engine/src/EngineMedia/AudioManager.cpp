...
bool AudioManager::Initialize(int maxChannels)
{
	...
	FMODCheckError(result);
	result = this->m_pSystem->setGeometrySettings(1000.0f);
	FMODCheckError(result);
...
void AudioManager::ClearChannel()
{
							...
							float minDistance, float maxDistance)
{
	...
	result = this->m_vecChannel[idChannel]->set3DAttributes(&FMODsoundPos, &FMODsoundVel);
	FMODCheckError(result);
